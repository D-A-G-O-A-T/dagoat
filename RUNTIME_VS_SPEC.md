...
| 16 | **Proxy revenue lane** (the allowlisted fetch network: three-signature receipts and their ten-stage off-chain verification, the gateway's meter commitment and strict-equality challenger, epoch aggregation, the settlement contracts, the node↔gateway tunnel crate, and the **operator-side sidecar** — destination allowlist decided on the resolved and pinned address, deny-net, robots, byte and schedule caps, consent gate, zero-content logging, 5 s kill switch, chunk-receipt metering) | `tools/goat-attestor/src/proxy/`, `contracts/src/proxy/`, `tools/goat-proxy-tunnel/`, `tools/goat-proxy-worker/`, and the operator surface in `desktop/src/proxy/` + `desktop/src-tauri/src/proxy/`. A **fifth** location, off the C1/C2/C3 deploy spine — Docker ships none of it, and the sidecar is a separate binary that the deploy spine neither builds nor links. | ✗ | ✗ | ✗ | **DESIGN** — the code exists and is gate-enforced, and **nothing about it runs**: no gateway is deployed, no consumer has ever transacted on it, no receipt has been verified outside a test, and no tunnel session has crossed anything but an in-process carriage. The sidecar has never been shipped in an installer, has never been spawned by the desktop shell, and **no session of it has crossed a real network** — every socket it has ever opened was a loopback fixture inside its own test suite, and its destination list is a placeholder of documentation domains rather than a cleared destination set. Its `User-Agent` now carries a ruled contact — founder decision D-6 is closed and the constant is `GoatCoin-Research-Fetcher/1.0 (+https://goatcoin.org/crawler-contact)`, with `crawler-contact@goatcoin.org` behind it — but **this document asserts nothing about whether that page resolves or that mailbox is read today**, and a header advertising a contact that goes nowhere is worse for abuse handling than one carrying none, because it is where a complaint arrives instead of a block. Standing both up, and keeping them up, remains a precondition for any real destination. Its per-release anti-malware submission is not delivered, and that blocks the first public installer. The one thing that genuinely executes is the **refusal path**, and refusing is not a capability. The desktop **Bandwidth** surface added on 2026-07-31 does not change that: it is hidden unless a sidecar binary sits beside the app **and** `GOAT_PROXY_PILOT=1`, which is true of no shipped build; it opens no socket from the webview and adds no CSP origin; and its spawn path refuses on **five** independent grounds before it starts anything (no signed record valid against the wallet the Rust process holds, the switch off, no destination manifest installed, no compile-time fingerprint for the sidecar, or a binary whose SHA-256 does not match it). Two of its numbers are honestly **absent rather than zero**: this build has no reader for the sidecar's event stream, so `bytes_today` is `null` and the open-socket count is `Unverified`, and the screen prints "not observed" rather than `0`. The third fact recorded here was a digest divergence — the desktop computed the consent record's `allowlist_digest` over the **disclosure list the operator read** while the sidecar computed it over its own **operational manifest**, so the two never agreed and the sidecar refused every record this surface wrote. That is **resolved**: the founder ruled the operator-facing SHA-256 summary digest (entry id and host, and nothing else) authoritative on both sides, and the sidecar now computes and verifies exactly that, pinned in its own suite against the desktop's parity fixture rather than against a second copy of its own arithmetic. Two consequences are stated rather than buried. First, what the signature covers is the set of **destinations**, which is what the disclosure names. Second, the sidecar's operational manifest — match mode, path scope, path prefixes and the per-entry rate — is deliberately **outside** the signed binding, so an edit to any of them no longer invalidates a signature and no longer refuses startup; that is the trade the ruling accepts so that daemon tuning cannot silently retire every operator's consent. The two lanes agreeing on the algorithm is not the same as their agreeing on the data: the disclosure list and the sidecar's destination manifest are still two separate artifacts with two separate id vocabularies, and a real pilot needs them issued together. The settlement contracts are inert behind a one-way `armed` flag that is off, and there are zero public deployments. Nothing on this path issues supply and nothing on it destroys supply. | "The fetch network is live / operational / open for sign-ups" · "the sidecar is running / installed / shipping" · "operators are compensated for bandwidth" or "run a node and be rewarded" as a present-tense capability · "the 90/10 split divides what a consumer paid" — under the shipped funding option it divides a funder-provided grant of GOAT that already exists, and consumer USDT is not distributed to any operator in any amount · any present-tense contribution-reward language attached to this lane |
