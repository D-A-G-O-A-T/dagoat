...
export default function ImportWallet({ goTo, finish, setWizardCtx }) {
  ...
  return (
    <form className="wizard-step" onSubmit={submit}>
      ...
      {pw.length > 0 && pw.length < MIN_PW && <p className="error-text">Password must be at least {MIN_PW} characters.</p>}
      ...
      <div className="wizard-actions">
        <button type="submit" className="primary-cta" disabled={!canSubmit}>{busy ? "Importing…" : "Import"}</button>
      </div>
    </form>
  );
}
