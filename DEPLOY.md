...
Signing seeds (Council-1): **`GOATD_SIGNING_SEED` (64 hex secret) preferred.** Deterministic
...
- [x] **C-6 THE REGRESSION GATE.** `handshake_and_gossip_round_trip_api_contract` +
      `aes_channel_round_trips_and_rejects_tamper` green (both re-runnable under `cargo test`).
...
- [x] **C-9 Live smoke / wire proof (blocking before Alpha — Council-2 + Council-4).**
      1. **Logical sizes (unchanged):** Initiation ≈ **3185 B**; cookie reply = **41 B**;
         CookieEcho ≈ **6534 B**; Response ≈ **6398 B**; SecureFrame ≈ **~6 KB**.
      2. **MTU mitigation (Council-4):** application-layer chunking (`datagram_framing`, tag `0xF1`)
...
- [x] **C-10 Sign-off.** Resolved versions (from committed `Cargo.lock`, Council-3):  
...
for a public bind, enforced before the socket is opened (see `SECURITY_QUICKFIXES.md`):
...
**Post–Track C / Council 1–3 residuals:**
...
| Deterministic testnet seeds | Gated (Council-1); Alpha needs `GOATD_SIGNING_SEED` |
| WAN MTU / IP fragmentation | Operational bound (Council-2) |
